-- ============================================================
-- POSTSPARK - SETUP COMPLETO DO SISTEMA DE CRÉDITOS (SPARKS)
...
COMMENT ON TABLE profiles IS 'Perfis de usuários estendendo auth.users com plano e saldo de sparks';
COMMENT ON TABLE spark_transactions IS 'Histórico de todas as transações de débito/crédito de sparks';
COMMENT ON TABLE generation_sessions IS 'Rastreia tentativas de geração e uso de regenerações por post';
COMMENT ON TABLE rate_limits IS 'Dados de rate limiting para proteção da API';
COMMENT ON FUNCTION debit_sparks IS 'Debita sparks do saldo do usuário atomicamente se houver saldo suficiente';
COMMENT ON FUNCTION use_regeneration IS 'Verifica e marca regeneração como usada para uma sessão de geração';
COMMENT ON FUNCTION refill_monthly_sparks IS 'Recarrega sparks mensais para usuários de planos pagos';

-- ============================================================
-- 7. USUÁRIOS DE TESTE (OPCIONAL - REMOVER EM PRODUÇÃO)
-- ============================================================

-- Para criar usuários de teste, execute no Auth > Users do Supabase Dashboard:
-- 1. Crie um usuário com email: dev@postspark.com
-- 2. Depois execute esta query para dar acesso DEV:
-- UPDATE profiles SET plan = 'DEV', sparks = 999999 WHERE email = 'dev@postspark.com';
...
-- Verificar tabelas:
-- SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('profiles', 'spark_transactions', 'generation_sessions', 'rate_limits');

-- Verificar funções:
-- SELECT proname FROM pg_proc WHERE pronamespace = 'public'::regnamespace AND proname IN ('debit_sparks', 'credit_sparks', 'use_regeneration', 'handle_new_user');
